...
パッケージはaptによるインストールとpipによるインストールを使用しており、順番を変更すると動かなくなる部分が多々あります。今回はなるべく自動でサーバーを構築したかったためplaybookが長くなりましたが、pythonのインストールを手動で実施したり、パッケージのインストールだけ別で作成する等の工夫でもう少しシンプルにできます。
